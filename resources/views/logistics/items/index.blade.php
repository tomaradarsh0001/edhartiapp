            ...
            <tbody>
                @foreach($items as $key => $item)
                    <tr>
                        ...
                        <td class="editablename" data-type="text" data-name="title" data-pk="{{$item->id}}"  style="color: {{ $status === 'inactive' ? '#ccc' : 'inherit' }};" {{ $status === 'inactive' ? 'data-bs-toggle=tooltip title=Disabled' : '' }}>
                            {{$item->name}}</i>
                        </td>
                        <td style="color: {{ $status === 'inactive' ? '#ccc' : 'inherit' }};" {{ $status === 'inactive' ? 'data-bs-toggle=tooltip title=Disabled' : '' }}>{{$item->LogisticCategory->name}}</td>
                        <td>
                        ...
                        @can('items.status')
                            <td>
                                <a href="{{ route('logistic.edit', $item->id) }}">
                                    <button type="button" class="btn btn-primary px-5" {{ $isDisabled ? 'disabled' : '' }}>Edit</button>
                                </a>
                            </td>
                        @endcan
                    </tr>
