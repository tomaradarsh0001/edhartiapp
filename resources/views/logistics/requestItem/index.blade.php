                ...
                <tbody>
                    @foreach ($logisticRequests as $requestId => $requests)
                                        ...
                                        <tr>
                                          ...
                                          <td>
                                                @php
                                                    $itemDetails = $requests->map(function ($request) {
                                                        return $request->logisticItem->name . ' (' . $request->requested_units . ')';
                                                    })->implode(', ');
                                                @endphp
                                                {{ $itemDetails }}
                                            </td>
                                            <td>
                                            @php
                                                $issuedUnits = $requests->map(function ($request) {
                                                    return $request->issued_units;
                                                });

                                                // Filter out null or zero values
                                                $issuedUnits = $issuedUnits->filter(function ($value) {
                                                    return $value !== null;
                                                });

                                                // Convert to a comma-separated string
                                                $issuedUnitsString = $issuedUnits->implode(', ');
                                            @endphp
                                            {{ $issuedUnitsString }}
</td>
                                            <td>
