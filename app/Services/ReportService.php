...
class ReportService
{
    ...
    {
        // dd($filter);
        $rows =  PropertyMaster::when(!empty($filter), function ($query) use ($filter) {
            if (isset($filter['colony'])) {
        ...
        if ($export) {
            ...
            $nonSplitCount = (clone $rows)->whereNull('is_joint_property')->count();

            $splitCount = (clone $rows)->withCount('splitedPropertyDetail')
                ->get()
                ->sum('splited_property_detail_count');

            // Total properties (non-split + split)
