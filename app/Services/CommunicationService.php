...
class CommunicationService
{
    ...
    public function sendWhatsAppMessage($data,$number,$action)
    {
        $configuration = Configuration::where('type', 'whatsapp')
        ->where('action', $action)
        ->where('status', 1)
        ->first();
        if ($configuration) {
    ...
    //to send text sms
    public function sendSmsMessage($data,$number,$action)
    {
        $configuration = Configuration::where('type', 'sms')
        ->where('action', $action)
        ->where('status', 1)
        ->first();
        if ($configuration) {
