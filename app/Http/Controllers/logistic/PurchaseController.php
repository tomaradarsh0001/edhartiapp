...
class PurchaseController extends Controller
{
    ...
    public function getPurchaseItems(Request $request)
    {
        ...
        $query = Purchase::query()
            ->select('purchases.purchase_id', 'purchases.purchased_date', 'purchases.logistic_items_id', 'purchases.purchased_unit', 'purchases.reduced_unit');
        ...
        if (!empty($request->input('search.value'))) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('purchases.purchase_id', 'LIKE', "%{$search}%")
                    ->orWhere('purchases.purchased_date', 'LIKE', "%{$search}%");
            });
        }
        ...
        $purchaseData = $query->offset($start)->limit($limit)->get();
        ...
        foreach ($purchaseData as $row) {
            ...
            if ($row->logistic_items_id) {
                $itemName = LogisticItem::where('id', $row->logistic_items_id)->pluck('name')->first();
                $itemName = $itemName ?? 'Unknown Item';
                $unit = $row->purchased_unit ?? $row->reduced_unit;
                $unitClass = $row->purchased_unit ? 'unit-green' : 'unit-red';
                $nestedData['items'] = $itemName . ' <span class="' . $unitClass . '">(' . $unit . ')</span>';
            } else {
                $nestedData['items'] = 'No Item';
            }
    ...
    public function getLogisticHistories(Request $request)
    {
        ...
        foreach ($stockData as $row) {
            ...
            $nestedData['last_added_date'] = $row->last_added_date ? Carbon::parse($row->last_added_date)->format('d/m/Y') : 'N/A';
            $nestedData['last_reduced_units'] = $row->last_reduced_units ?? '';
            $nestedData['last_reduced_date'] = $row->last_reduced_date ? Carbon::parse($row->last_reduced_date)->format('d/m/Y') : 'N/A';
            ...
            $nestedData['issued_at'] = $row->issued_at ? Carbon::parse($row->issued_at)->format('d/m/Y H:i:s') : 'N/A';
