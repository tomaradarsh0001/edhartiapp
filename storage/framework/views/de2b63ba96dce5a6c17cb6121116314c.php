...
<div class="bs-stepper-content">
    <div id="" role="" class="" aria-labelledby="">
        ...
        <?php echo $__env->make('include.parts.property-selector',['leaseHoldOnly'=>true], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
...
<form class="d-none" method="post" action="<?php echo e(route('createRgr')); ?>" id="rgr-card">
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="g-3">
            <button class="btn btn-primary px-4 mt-4" type="button" id="submitButton3">Calculate Ground Rent</button> 
...
<script>
    ...
    $('#submitButton3').click(function() {
        if ($(this).attr('type') == "button" && !editing) {
            ...
            if (property_id != "" && splited != "", fromDate != "") {
                $.ajax({
                    url: "<?php echo e(route('calculateGroundRent')); ?>",
                    method: 'post',
                    data: {
    ...
    function getPropertyBasicDetail(propId) {
        $.ajax({
            type: "post",
            url: "<?php echo e(route('propertyBasicdetail')); ?>",
            data: {
                _token: '<?php echo e(csrf_token()); ?>',
                property_id: propId
            },
    ...
    $('#reason_of_change').change(function() {
        ...
        if (reason != "") {
            if (reason == 1) { // when area change selected
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo e(route('checAreaChanged')); ?>",
                    data: {
                        _token: "<?php echo e(csrf_token()); ?>",
            ...
            if (reason == 2) { // when land rate change selected
                $.ajax({
                    type: "POST",
                    url: "<?php echo e(route('checkPropertyStatusChanged')); ?>",
                    data: {
                        _token: "<?php echo e(csrf_token()); ?>",
                        property_master_id: selectedRGR.property_master_id,
                        splited_property_detail_id: selectedRGR.splited_property_detail_id,
        ...
        function displayRgrFactorTable(factor, container) {
            if (Object.keys(factor).length > 0) {
                ...
                Object.keys(factor).forEach(key => {
                    tableHTML += `<th colspan="${Object.keys(factor[key]).length}">${key}</th>`;
                });
    ...
    function fillEditForm(reasonForChange, oldRgrId, fromDate, tillDate, editDate = null, oldArea = null, updatedArea = null) {
        ...
        editFormData.append('_token', "<?php echo e(csrf_token()); ?>"); // Append CSRF token
    ...
    function submitEditedForm() {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo e(route('saveEditedRGR')); ?>",
            data: editFormData,
    ...
    function appendPropertyDetail(data, isMultiple = false, rowNum = null) {
        ...
        if (data.trasferDetails && data.trasferDetails.length > 0) {
            ...
            <span class="qmark">&#8505;</span> <ul class="transfer-list">
                <li class="transfer-list-item">
                    <div class="transfer-list-cell">#</div>
                    <div class"transfer-list-cell">Transfer Date</div>
                    <div class="transfer-list-cell">Process </div>
                    <div class="transfer-list-cell">Lessee Name</div>
                    </li>
            `;
            data.trasferDetails.forEach((row, i) => {
                transferHTML += `<li class="transfer-list-item">
                    <div class="transfer-list-cell">${i+1}</div>
                    <div class="transfer-list-cell">${row.transferDate}</div>
                    <div class="transfer-list-cell">${row.process_of_transfer}</div>
                    <div class="transfer-list-cell">${row.lesse_name}</div>
                    </li>`
            });
            transferHTML + `</ul></div>`;
        }
...
</script><?php /**PATH C:\Users\shubh\Desktop\new 22 august\eDharti\resources\views/rgr/property-input.blade.php ENDPATH**/ ?>